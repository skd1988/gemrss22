...
function App() {
  ...
  const handleRedirectUrlSubmit = async (url) => {
    setError(null);
    if (!url.trim()){
  ...
  const fetchInoreaderFeed = async (url, credentials) => {
    const getStreamId = (inoreaderUrl) => {
        try {
  ...
  const loadAndSummarizeNews = async () => {
    ...
    try {
      ...
      const cacheData = { articles: categorizedArticles, timestamp: Date.now() };
  ...
  const handleSendMessage = async (message) => {
    if (!chatSession) {
      const errorBotMessage = { role: 'model', content: t('app.chatError', { error: "Chat session not initialized." }) };
    ...
    try {
      ...
      const newBotMessage = { role: 'model', content: response.text };
      setChatHistory(prev => [...prev, newBotMessage]);
    } catch (e) {
      const errorMessage = e instanceof Error ? e.message : 'An unknown error occurred.';
      const errorBotMessage = { role: 'model', content: t('app.chatError', { error: errorMessage }) };
      setChatHistory(prev => [...prev, errorBotMessage]);
    } finally {
      setIsChatLoading(false);
    }
